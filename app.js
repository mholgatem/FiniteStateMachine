...
function variableToken(name, val) {
  if (val === 'X') return 'X';
  const content = nameToSvg(name);
  if (val === '0') return `<tspan class="overline">${content}</tspan>`;
  return content;
}
...
function renderPalette() {
  ...
  unplaced.forEach((st) => {
    ...
    node.querySelector('.state-extra').textContent =
      state.type === 'moore'
        ? buildPlainIOText(state.outputs, st.outputs, state.showBinary ? 'binary' : 'vars')
        : '';
