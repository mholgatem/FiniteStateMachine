...
function buildTransitionColumnTemplates() {
  ...
  for (let i = bitCount - 1; i >= 0; i -= 1) {
    templates.push({ key: `q_${i}`, label: `Q_${i}`, type: 'value' });
  }
  ...
  for (let i = bitCount - 1; i >= 0; i -= 1) {
    nextStateTemplates.push({ key: `next_q_${i}`, label: `Q_${i}^+`, type: 'value' });
  }
  ...
  templates.push(...inputTemplates, ...nextStateTemplates, ...outputTemplates);
  templates.push({ key: 'spacer', label: '', type: 'spacer', allowMultiple: true });
  return templates;
}

function createSpacerColumn() {
  return { key: `spacer_${Date.now()}_${Math.random().toString(16).slice(2)}`, label: '', type: 'spacer' };
}
...
function buildDefaultTransitionColumns(templates) {
  ...
  addSpacer();
  currentStates.forEach((t) => columns.push({ ...t }));
  if (inputs.length) addSpacer();
  inputs.forEach((t) => columns.push({ ...t }));
  addSpacer();
  nextStates.forEach((t) => columns.push({ ...t }));
  if (outputs.length) addSpacer();
  outputs.forEach((t) => columns.push({ ...t }));
  return columns;
}
...
function ensureTransitionTableStructure() {
  ...
  const validValueKeys = new Set(templates.filter((t) => t.type === 'value').map((t) => t.key));
  state.transitionTable.columns = state.transitionTable.columns.filter((col) => {
    if (col.type === 'spacer') return true;
    return validValueKeys.has(col.key);
  });
  ...
  transitionDrawer.addEventListener('drop', (e) => {
    ...
    if (payload.source === 'selection' && !Number.isNaN(payload.fromIndex)) {
      ...
      if (template.type === 'spacer') {
        ...
        const existingIdx = columns.findIndex((col) => col.key === template.key && col.type === template.type);
        if (existingIdx !== -1) {
          const [col] = columns.splice(existingIdx, 1);
          if (index > existingIdx) index -= 1;
          columns.splice(Math.max(0, Math.min(columns.length, index)), 0, col);
        } else {
          columns.splice(Math.max(0, Math.min(columns.length, index)), 0, { ...template });
        }
      }
    }
